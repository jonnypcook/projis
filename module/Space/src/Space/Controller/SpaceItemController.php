...
class SpaceitemController extends SpaceSpecificController
{
    ...
    public function indexAction()
    {
        ...
        $q = $this->getEntityManager()->createQuery('SELECT s.spaceId, s.name FROM Space\Entity\Space s JOIN s.building b WHERE s.project=:project AND s.deleted!=true AND s.root=0 ORDER BY b.name, s.name ASC')
        ...
        try {
            ...
            if (!empty($post['qMode'])) {
                ...
                if (empty($maximumPhosphorLength)) {
                    $errors['maxphosphorlength'] = array("isEmpty" => 'maximum phosphor unit length not provided via parameter or product');
                }
