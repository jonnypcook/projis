...
use DoctrineORMModule\Proxy\__CG__\Project\Entity\Project;
...
class ProjectController extends AuthController
{
    ...
    /**
     * Exclude from reporting action
     *
     * This will exclude all selected items from reporting
     */
    public function excludeReportingAction()
    {
        ...
        if ( $request->isPost() )
        {
            $excludeItems = $post['exclude'];

            foreach ( $excludeItems as $pid )
            {
                $project = $this->getEntityManager()->getRepository( 'Project\Entity\Project' )->findByProjectId( $pid );
                $project->setExcludeReporting( 1 );
        ...
        $this->flashMessenger()->addSuccessMessage('Selected projects has been excluded successfully!');

        return $this->redirect()->toRoute('ratingreport', array('action' => 'view', 'rating' => $rating, 'user' => $user ));
    }
}