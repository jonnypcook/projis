...
class ReportTotalController extends AuthController
{
    ...
    public function viewAction()
    {
        ...
        switch ( strtolower( $type ) )
        {
            ...
            case 'clients':
                $this->getView()->setVariable( 'title', 'Total New Clients' );
                $data = $this->getClients( $user );
                break;
            case 'contacts':
                $this->getView()->setVariable( 'title', 'Total New Contacts' );
                $data = $this->getContacts( $user );
                break;
            case 'quotations':
                $this->getView()->setVariable( 'title', 'Total Quotations' );
                $data = $this->getQuotations( $user );
                break;
            case 'proposals':
                $this->getView()->setVariable( 'title', 'Total Proposals Raised' );
            ...
            case 'allprojects':
                $this->getView()->setVariable( 'title', 'Total Projects' );
                $data = $this->getAllProjects( $user );
                break;
            case 'wonprojects':
                $this->getView()->setVariable( 'title', 'Total Won Projects' );
                $data = $this->getWonProjects( $user );
                break;
            case 'lostprojects':
                $this->getView()->setVariable( 'title', 'Total Lost Projects' );
                $data = $this->getLostProjects( $user );
                break;
            case 'openprojects':
                $this->getView()->setVariable( 'title', 'Total Open Projects' );
        ...
        if ( !empty($user_id) && $user_id )
        {
            $sql
                = 'SELECT
                    ...
                    WHERE (`Client`.user_id=' . $user_id . ') and
                     Client.`created` >= \'' . date( 'Y-m-d H:i:s', $dt ) . '\'
            ...
            $sql
                = 'SELECT
                    ...
                    FROM `Project`
                      ...
                      JOIN `User` u
                        ON u.user_id = `Client`.user_id
                        WHERE Client.`created` >= \'' . date( 'Y-m-d H:i:s', $dt ) . '\'
                    GROUP BY client_name
                    ORDER BY Client.created DESC';
        }
    ...
    private function getContacts( $user_id )
    {
        ...
        if ( !empty($user_id) )
        {
            ...
            $qb->select( Array(
                ...
                ->from( 'Contact', 'c' )
                ->innerJoin( 'c', 'Client', 'cl', 'cl.client_id = c.client_id' );
            $sql
                = 'SELECT
                    c.contact_id, cl.client_id, c.forename, c.surname, cl.name client_name,
                     CONCAT(u.forename, " ", u.surname) user_name,
            ...
            $sql
                = 'SELECT
                    ...
                    WHERE
                        ...
                        d.`created` >= \'' . date( 'Y-m-d H:i:s', $dt ) . '\' AND
                        c . `user_id` = ' . $user_id . '
            ...
            $sql
                = 'SELECT
                    ...
                    where
                        ...
                        p.`contracted` >= \'' . date( 'Y-m-d H:i:s', $dt ) . '\' AND
                        c . `user_id` = ' . $user_id . '
                    GROUP BY p.project_id
            ...
            $sql
                = 'SELECT
                    ...
                    where
                        p.contracted IS NOT NULL AND
                        ps.`weighting` = 1 AND
                        p.`contracted` >= \'' . date( 'Y-m-d H:i:s', $dt ) . '\'
                    GROUP BY p.project_id
                    ORDER BY p.contracted DESC';
        }
